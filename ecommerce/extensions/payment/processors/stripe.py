...
class Stripe(ApplePayMixin, BaseClientSidePaymentProcessor):
    ...
    def create_new_payment_intent_for_basket(self, payment_intent_id, basket):
        """
    ...
    def handle_processor_response(self, response, basket=None):
        ...
        if dynamic_payment_methods_enabled:
            if confirm_api_response['status'] == 'requires_payment_method':
                logger.info(
                    'Dynamic Payment Method received requires a payment method attached for edX order %s,'
                    'returning %s Payment Intent confirmation status to the payment MFE',
